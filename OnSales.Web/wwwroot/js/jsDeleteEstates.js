...
let idDeleteMunicipality;


let DeleteMunicipality = function (entity, element, id) {
    idDeleteMunicipality = id;

    console.log(idDeleteEstate)

    document.getElementById('cmdDelete').addEventListener('click', () => {
        console.log(id);
        ConfirmDeleteMunicipality();
    })
...
let ConfirmDeleteMunicipality = function () {
    $.ajax({
        type: "POST",
        url: `${urlApoyo}/DeleteConfirmedMunicipality`,
        data: { id: idDeleteMunicipality }
    }).done(function (data) {
        document.getElementById(`row_${data.id}`).remove();
        var toastHTML = '<span>Registro eliminado</span><button class="btn-flat toast-action"><i class="material-icons">check</i></button>';
        M.toast({ html: toastHTML });
